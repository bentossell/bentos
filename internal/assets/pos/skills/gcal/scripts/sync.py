...
def main() -> int:
	...
	if not account:
		emit({'type': 'error', 'message': 'gcal account not configured in SKILL.md frontmatter'})
		return 1
	...
	emit({'type': 'progress', 'message': f'gccli events {calendar_id}', 'pct': 0.1})
	...
	state = {
		...
		'raw': None if events else proc.stdout.strip()[:2000],
	}
	...
	emit({'type': 'result', 'ok': True, 'data': {'events': len(events), 'account': account}})
	return 0
